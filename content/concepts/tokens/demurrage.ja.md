...
1. <span class="code-snippet">( e ^ (t ÷ τ) )</span>の値を計算する。この数値を「デマレージ係数」と呼ぶ。デマレージ係数は常に現在時刻など特定の時刻からの相対値である。
2. 変換する量に適用します。
...
1. 最初の8ビットは <span class="code-snippet">0x01</span> でなければなりません。
...
5. 次の24ビットは予約されており，すべて<span class="code-snippet">0</span>でなければなりません
...
表示値からレジャー値に変換するには、<span class="code-snippet">Amount.from_human()</span>を使用する。
