...
export const sortDates = (arr, upcomingFirst = true) => {
  return arr.sort((a, b) => {
    const dateA = moment(a.end_date, "MMMM D, YYYY");
    const dateB = moment(b.end_date, "MMMM D, YYYY");
    return upcomingFirst ? dateA.diff(dateB) : dateB.diff(dateA); // Returns a negative value if dateA is before dateB, positive if after, and 0 if the same
  });
};
...
function categorizeDates(arr) {
  ...
  return { past: sortDates(past, false), upcoming: sortDates(upcoming, true) };
}
...
const events = [
...

  {
    ...
    date: "TBD",
    image: require("../static/img/events/event-meetup-toronto@2x.jpg"),
    end_date: "January 31, 2025", // putting future date as it is still in TBD start date.
  },
