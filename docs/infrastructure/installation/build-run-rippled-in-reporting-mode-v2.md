...
{% admonition type="warning" name="Reporting mode deprecated" %}
Reporting mode was deprecated and removed in `rippled` version 2.3.0, released Nov 25, 2024. This functionality has been replaced by the Clio server, and it is recommended that you migrate your infrastructure to use the Clio server instead.
{% /admonition %}
...
Clio can be configured to store data in [Apache Cassandra](https://cassandra.apache.org/_/index.html) or [ScyllaDB](https://www.scylladb.com/), enabling scalable read throughput. Multiple Clio nodes can share access to the same dataset, which allows for a highly available cluster of Clio nodes without the need for redundant data storage or computation.

To learn more about Clio and how to run it, see [The Clio Server](../../concepts/networks-and-servers/the-clio-server.md).

## Clio vs. `rippled` Reporting Mode (Deprecated)

The Clio server is designed to replace the now deprecated `rippled` Reporting Mode. It provides a more efficient, scalable, space-saving solution for serving validated XRP Ledger data via WebSocket and HTTP APIs.

The key differences between Clio and `rippled` Reporting Mode are:

- **Storage model**
  - Clio uses Cassandra or ScyllaDB, and focuses on efficient ledger data storage.
  - `rippled` Reporting Mode ingests data into Postgresql for structured querying, but also uses Cassandra/NuDB for ledger history.
- **Scalability**
  - Clio enables easier horizontal scaling, with multiple Clio servers sharing a dataset without redundancy.
  - `rippled` Reporting Mode allows multiple servers to share a PostgreSQL database, but only one server writes at a time.
- **Performance**
  - Clio is optimized for high-throughput API access with lower latency.
  - `rippled` Reporting Mode is structured for efficient API queries, but relies on a relational database, which may introduce overhead.


## Migrating from `rippled` Reporting Mode to Clio
